...
def plot_stoc(src_dir_name, dst_name, format='png'):
    comm_budget_list_DOCO = [800, ] + [i for i in range(1200, 6000, 600)] + [i for i in range(6000, 60000, 6000)] + \
                            [i for i in range(60000, 300000, 60000)] + [420000, ] + \
                            [i for i in range(600000, 1800001, 600000)]
...
def plot_adv_32_8(src_dirname_8, src_dirname_32, dst_name='', format='png'):
    ...
    # # legend1 = plt.legend(handles=patches, columnspacing=0.4, bbox_to_anchor=(0.82+0.22, 1.35-0.02), ncol=3, frameon=False, fontsize=20)
    ...
    plt.xlabel("Communication Budget", fontsize=23)
    plt.ylabel("Error Rate", fontsize=23)
    # plt.title("(d) ${\\mathtt{epsilon}}$ (clique)", y=-0.5, font={'family': 'Helvetica', 'size': 27})
    plt.title("($\\mathtt{d}$) ${\\mathrm{epsilon}}$ ($\\mathtt{clique}$)", y=-0.55,
