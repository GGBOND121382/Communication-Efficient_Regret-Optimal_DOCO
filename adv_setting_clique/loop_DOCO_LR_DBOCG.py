...
def run_comm():
    ...
    for comm_bueget in comm_budget_list:
        ...
        plot_filename = './plot_data_' + filename[:3] + '_' + repr(number_of_clients) + '/DBOCG_' + filename + '_' + repr(comm_bueget)
        ...
        loss, class_err, comm_cost = DBOCG(grad_logReg, data_collection, target_collection, number_of_clients,
                                           ...
                                           selected_learner=-1, stepsize_factor=best_index_list_DBOCG[repr(comm_bueget)], is_clique=True)
...
def run_time():
    ...
    for i in range(1, 6):
        ...
        data_collection, target_collection = load_data_collection_interval_dist(original_data_collection,
                                                                                ...
                                                                                number_of_clients)
        target_collection = target_adversarize(target_collection, number_of_clients, kb=3)
        comm_budget_list = [int((number_of_clients - 1) * number_of_clients * running_time_horizon / j) + 1 for j in range(1, 4)]
        for comm_bueget in comm_budget_list:
            for stepsize_factor in [0.01, 0.1, 1, 10, 100, 1000, 10000, 100000]:
            # for stepsize_factor in [10000, 100000]:
            # for stepsize_factor in [1, 10]:
                plot_filename = './plot_data_' + filename[:3] + '_' + repr(number_of_clients) + '/DBOCG_' + filename + '_' + repr(comm_bueget) + '_' + repr(running_time_horizon)
                fd = open(plot_filename, 'a')
                ...
                loss, class_err, comm_cost = DBOCG(grad_logReg, data_collection, target_collection, number_of_clients,
                                                   2, mat_A, comm_bueget, radius=radius, is_l2_norm=True,
                                                   selected_learner=-1, stepsize_factor=stepsize_factor, is_clique=True)

                for i in range(number_of_clients):
                    fd.write(repr(loss[i]) + ' ' + repr(class_err[i]) + ' ' + repr(comm_cost) + ' ' + repr(stepsize_factor) + '\n')
