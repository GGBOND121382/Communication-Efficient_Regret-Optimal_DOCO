...
def run_comm():
    ...
    for comm_bueget in comm_budget_list:
        plot_filename = './plot_data_' + filename[:3] + '_' + repr(number_of_clients) + '/DB_TDOCO_' + filename + '_' + repr(comm_bueget)
...
def run_time():
    ...
    endTime = time.time()
    print("Data loaded: " + repr(endTime - startTime) + "seconds")
    print("y", original_target_collection[0][:10, :])

    unit_time_step = int(time_horizon / 5)
    for i in range(1, 6):
        running_time_horizon = i * unit_time_step
        data_collection, target_collection = load_data_collection_interval_dist(original_data_collection, original_target_collection, dimension, 0, running_time_horizon, number_of_clients)
        target_collection = target_adversarize(target_collection, number_of_clients, kb=3)
        for comm_bueget in comm_budget_list:
            for stepsize_factor in [0.01, 0.1, 1, 10, 100, 1000, 10000, 100000]:
            # for stepsize_factor in [10000,]:
                plot_filename = './plot_data_' + filename[:3] + '_' + repr(number_of_clients) + '/DB_TDOCO_' + filename + '_' + repr(comm_bueget) + '_' + repr(running_time_horizon)
                fd = open(plot_filename, 'a')

                loss_mean, class_err_mean, comm_cost = DB_TDOCO(grad_logReg, data_collection, target_collection, number_of_clients,
                                                                number_of_clients, comm_bueget, radius=radius, is_l2_norm=True, stepsize_factor=stepsize_factor)
                print(f'filename = {filename}, comm_budget = {comm_bueget}, loss_mean = {loss_mean},'
                      f' class_err_mean={class_err_mean}, comm_cost = {comm_cost}')

                fd.write(repr(loss_mean) + ' ' + repr(class_err_mean) + ' ' + repr(comm_cost) + ' ' + repr(stepsize_factor) + '\n')
                fd.close()
