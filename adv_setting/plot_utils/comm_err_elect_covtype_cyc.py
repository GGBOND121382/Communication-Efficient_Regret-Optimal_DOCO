...
def election_adv(number_of_clients, src_dir_name, base_err_list, dst_name, filename="covtype.libsvm.binary"):
    ...
    for i in range(1, 6):
        ...
        for comm_budget in comm_budget_list_TDOCO:
            file_name = src_dir_name + 'DB_TDOCO_' + filename + '_' + repr(comm_budget) + '_' + repr(running_time_horizon)
            lossmean_list = []
            class_err_list = []
    ...
    for i in range(1, 6):
        ...
        for comm_budget in comm_budget_list_DOCO:
            file_name = src_dir_name + 'DOCO_gossip_' + filename + '_' + repr(comm_budget) + '_' + repr(running_time_horizon)
            lossmean_list = []
            class_err_list = []
            with open(file_name) as fd:
                for line in fd.readlines():
    ...
    ############################################################
    comm_err_dict = {'base_err_list': base_err_list, 'DB_TDOCO_comm_budget': TDOCO_delay_err_mean, 'gossip_comm_budget': DOCO_gossip_err_mean,
...
if __name__ == '__main__':
    ...
    # cycle network
    src_dir_name = 'plot_data_purify/adv_setting/'
    ...
    election_adv(32, src_dir_name, base_err_list, 'adv_setting')
