...
#     dirname = '../data/non_iid_data/'
#     num_of_clients = 32
#     data_filename = 'covtype.libsvm.binary'
#     rpt_times = 1

def config_data_numLearners_rpt(data_filename, num_of_clients, rpt_times=1, isForTime=False):
    dirname = '../data/non_iid_data/'
    ...
    if(num_of_clients == 32):
        comm_budget_list = [i for i in range(100, 6000, 100)] + [i for i in range(6000, 24000, 6000)] + [20000, ] + \
                           [i for i in range(24000, 60000, 6000)] + [i for i in range(60000, 120001, 60000)]
    else:
        comm_budget_list = [i for i in range(100, 6000, 100)] + [i for i in range(6000, 60000, 6000)] + \
                           [i for i in range(60000, 120001, 60000)]

    if(isForTime):
        comm_budget_list = [i for i in range(100, 6000, 100)] + [i for i in range(6000, 60000, 1000)] + \
